...
1. **Autonomy is opt-in, not ambient.** No skill, hook, or agent self-schedules or starts work on its own — opening the laptop or starting Claude/Codex never begins work or spends a token. But the user may stand up any number of scheduled/unattended automations (Codex Automations, Task Scheduler, cron) for named jobs, and Claude never blocks their creation or operation. Two standing safeguards replace the old blanket ban: **(1) the automation budget circuit breaker** — aggregate automation token spend is capped per day (default 10M tokens/day across all Codex automations); when the day's ledger reaches the ceiling, every automation's preflight aborts and does no work until the next day or a the user reset; **(2) the global kill-switch** — a `_system/automations/HALT` file (or `budget.json` ceiling set to 0) halts all automations on next preflight. All automations run on **subscription credits only — never metered API credits.** Every standing automation is recorded in `_system/automations/REGISTRY.md` for visibility (not as an approval gate). A skill deciding on its own to run on a schedule, with the user not standing it up, remains banned. Irreversible actions — auto-merge, public push outside the guard gate, spend past the ceiling — remain forbidden to automations.
