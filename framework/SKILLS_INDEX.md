...
| **loop-engineering** | Owain Lewis two-loop autonomous coding system. Manager loop triages the GitHub backlog (classify risk/type, mark agent-ready) + files evidence-backed tickets; worker loop turns agent-ready tickets into draft PRs (plan → fix → reviewer subagent → draft PR, never merge). Both Codex-run via Codex Automations (subscription), budget-gated by `_system/automations/` (10M/day breaker + HALT kill-switch). Wrenches: backlog-manager, issue-to-pr |
