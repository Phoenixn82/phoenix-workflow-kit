...
2. **Simplicity first.** Minimum code that solves the problem. No features beyond what was asked. No abstractions for single-use code. No "flexibility" that wasn't requested. If you write 200 lines and it could be 50, rewrite.
...
**No stale shell-folder roots — ever.** The user's shell-folder registry can point at dead synced paths. NEVER use `[Environment]::GetFolderPath('Desktop'|'MyDocuments'|'MyPictures')` for deliverables without validating it points at the live folder. Hardcode the real roots: Desktop `C:\Users\<you>\Desktop`, Documents `C:\Users\<you>\Documents`, Pictures `C:\Users\<you>\Pictures`. If the registry still redirects those folders elsewhere, offer to clean it up but don't rewrite shell-folder registry or move files unprompted.
